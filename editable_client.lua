...
-- RegisterNetEvent('mech:setJob')
-- AddEventHandler('mech:setJob', function(job)
--     PlayerData.job = job
-- end)

RegisterNetEvent('mech:playerLoaded')
AddEventHandler('mech:playerLoaded', function()
    TriggerCallback("kaves_mechanic:server:getVehData", false, function(data)
		if not data then
			return 
		end
		vehData = data
...
TriggerCallback = function(cbName, cb, data)
    lib.callback(cbName, false, function(output)
        print(output)
        if cb then
            cb(output)
        else
